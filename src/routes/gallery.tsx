...
export const Route = createFileRoute("/gallery")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Real smile transformations and a glimpse inside our modern dental clinic.",
      },
    ],
  }),
...
function GalleryPage() {
  ...
  return (
    <>
      <section className="pt-32 pb-12 bg-gradient-hero">
        <div className="mx-auto max-w-7xl px-6 text-center">
          ...
          <h1 className="font-display text-5xl md:text-7xl font-bold text-navy">
            Beautiful Smiles, <span className="text-gradient-primary">Real Results</span>
          </h1>
          <p className="mt-6 text-lg text-muted-foreground max-w-2xl mx-auto">
            Explore our smile transformations and the modern environment behind every patient's
            experience.
          </p>
        </div>
      </section>
      ...
      {lightbox && (
        <div
          className="fixed inset-0 z-[60] bg-navy/95 flex items-center justify-center p-6 animate-fade-up"
          onClick={() => setLightbox(null)}
        >
          <button
            className="absolute top-6 right-6 text-white p-2 rounded-full glass-dark"
            aria-label="Close"
          >
            <X />
          </button>
          <img
            src={lightbox}
            alt=""
            className="max-h-[90vh] max-w-[90vw] rounded-2xl shadow-elegant"
          />
        </div>
      )}
      ...
      <section className="py-20">
        <div className="mx-auto max-w-4xl px-6 text-center">
          <SectionHeading
            ...
            title={
              <>
                Ready for Your <span className="text-gradient-primary">Dream Smile?</span>
              </>
            }
          />
